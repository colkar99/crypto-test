  ...
  task :liability_proof => :environment do
    Account.currency.values.each do |type|
      ...
      if proof.coin?
        ...
        begin
          if type == 'eth'
            doc = open "https://etcchain.com/api/v1/getAddressBalance?address=" << addresses, redirect: false
          ...
          proof.addresses = []
          puts "#{type} is not supported yet. Unable to fetch address balances automatically."
        end
      end
